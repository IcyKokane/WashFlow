...
type View = 'home' | 'customer' | 'lead' | 'estimate' | 'estimateDetail' | 'jobs' | 'jobDetail'

const initialData: ServiceCoreData = {
  customers: [], properties: [], leads: [], estimates: [], jobs: [], jobPhotos: [], serviceTemplates: pressureWashingServices
}
...
export default function App() {
  ...
  const addCustomer = (event: FormEvent<HTMLFormElement>) => {
    ...
    const form = new FormData(event.currentTarget); const customerId = newId('customer'); const propertyId = newId('property'); const now = new Date().toISOString()
  ...
  const addEstimate = (event: FormEvent<HTMLFormElement>) => {
    ...
    setData((current) => ({ ...current, leads: current.leads.map((item) => item.id === lead.id ? { ...item, status: 'quoted' } : item), estimates: [...current.estimates, { id: estimateId, leadId: lead.id, customerId: lead.customerId, propertyId: lead.propertyId, items, subtotal: estimateSubtotal(items), status: 'draft', createdAt: new Date().toISOString() }] }))
    setSelectedEstimateId(estimateId); setView('estimateDetail')
  }

  const setEstimateStatus = (status: 'sent' | 'declined') => setData((current) => ({ ...current, estimates: current.estimates.map((item) => item.id === selectedEstimateId ? { ...item, status } : item) }))

  const acceptEstimate = () => {
    const estimate = data.estimates.find((item) => item.id === selectedEstimateId); if (!estimate) return
    const existingJob = data.jobs.find((job) => job.estimateId === estimate.id)
    if (existingJob) { setSelectedJobId(existingJob.id); setView('jobDetail'); return }
    const job = createJobFromEstimate(estimate)
    setData((current) => ({ ...current, estimates: current.estimates.map((item) => item.id === estimate.id ? { ...item, status: 'accepted', acceptedAt: new Date().toISOString() } : item), leads: current.leads.map((item) => item.id === estimate.leadId ? { ...item, status: 'accepted' } : item), jobs: [...current.jobs, job] }))
    setSelectedJobId(job.id); setView('jobDetail')
  }

  const scheduleJob = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); const form = new FormData(event.currentTarget); const scheduledFor = String(form.get('scheduledFor') || '')
    if (!scheduledFor) return
    setData((current) => ({ ...current,
      jobs: current.jobs.map((job) => job.id === selectedJobId ? { ...job, scheduledFor, status: 'scheduled' } : job),
      leads: current.leads.map((lead) => { const job = current.jobs.find((candidate) => candidate.id === selectedJobId); return job && lead.propertyId === job.propertyId && lead.status === 'accepted' ? { ...lead, status: 'scheduled' } : lead })
    }))
  }
  ...
  const addPhoto = async (event: ChangeEvent<HTMLInputElement>, kind: JobPhotoKind) => {
    ...
    try {
      ...
      setData((current) => ({ ...current, jobPhotos: [...current.jobPhotos, { id: newId('photo'), jobId: job.id, propertyId: job.propertyId, kind, dataUrl, createdAt: new Date().toISOString() }] }))
